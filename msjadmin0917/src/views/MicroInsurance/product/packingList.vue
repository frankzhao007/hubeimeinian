      ...
      <div class="inquire">
        ...
        <el-row style="margin-top:10px;">
          <el-col :span="6">
            ...
            <div class="grid_content">
              <el-select :disabled="!AddProjectCompany||AddProjectCompany=='全部'||AddProjectCompany==''||AddProjectCompany=='优健康自营'||AddProjectCompany=='微保'" filterable clearable v-model="creatpeople" placeholder="请选择" size="medium" style="width:100%;">
...
export default {
  ...
  watch: {
    AddProjectCompany(nvl) {
      ...
      if (!nvl || nvl == "" || nvl == "全部" || nvl == "优健康自营"|| nvl == "微保") {
      } else {
        this.$api
...
//    this.Filialelist1 = [];
//    this.AddProjectCompany = this.$store.getters.getRoleInfo.AccountName;
...
//  }
  ...
  methods: {
    ...
    ChangeAddProjectSetSalePrice() {
      ...
      var body = {
        ...
        gooddiscount: Number(
          (
            Number(this.Discount.DiscountPrice) /
            Number(this.selectAddProjectobj.goodprice)*10
          ).toFixed(1)
        ),
      ...
      this.$api.post("/manage/setWeiBaoSalePrice", body).then(res => {
        console.log(res);
        if (res.code == "200") {
    ...
    manageQueryAddPackIndex() {
      ...
      this.$api.post("/manage/queryWeiBaoAddPack", body).then(res => {
        console.log(res);
    ...
    //新增加项包按钮
    addAddPackageBtn() {
      this.$router.push("/MicroInsurance/packageadd");
    },
    //编辑加项包按钮
    editAddPackageBtn(val) {
      this.$router.push("/MicroInsurance/packageadd?goodsid=" + val);
    },
    //copy加项包按钮
    copyAddPackageBtn(val) {
      this.$router.push(
        "/MicroInsurance/packageadd?goodsid=" + val + "&isedit=false"
      );
    },
    //新增加项包确认
    ...
    DeleteAddPackageBtn(value) {
      ...
      this.$confirm("确认要删除该加项包吗？", {
        ...
        .then(() => {
          this.$api
            .get("/manage/deleteWeiBaoAddPackage", { goodid: value.goodid })
            .then(res => {
              if (res.code == "200") {
                this.$message({
                  type: "success",
    ...
    ChangeAddPackageStatusBtn(value) {
      ...
      this.$confirm("确认要" + goodsstatus + "吗？", {
        ...
        .then(() => {
          this.$api
            .post("/manage/putWayWeiBaoAddPackage", {
              goodid: value.goodid,
