<template>
  <div style="margin: 20px">
    <div
      class="checkSchedule"
      v-if="item.scheduleName == activeName"
      v-for="(item, index) in MSJschedules"
    >
              ...
              >
                ...
                >
                  <el-tooltip placement="top">
                    <div slot="content">
                      多行信息多行信息多行信息<br />第二行信息<br />第二行信息
                    </div>
                    <div
                      class="word"
                      style="float: left; width: 33%; text-align: center"
                    >
                      <span v-if="matchingdata(days, data.day)">{{
                        matchingdata(days, data.day).numberMorning
                      }}</span>
                      <span v-else>-</span>
                    </div>
                  </el-tooltip>
                  ...
                  <div style="float: left; width: 34%; text-align: center">
                    <span v-if="matchingdata(days, data.day)">{{
                      matchingdata(days, data.day).alreadyNumberM
                    }}</span>
                    <span v-else>-</span>
                  </div>
                  <div style="float: left; width: 33%; text-align: center">
                    <span v-if="matchingdata(days, data.day)">{{
                      matchingdata(days, data.day).stockNumberM
                    }}</span>
                    <span v-else>-</span>
                  </div>
                </div>
                ...
                >
                  <div style="float: left; width: 33%; text-align: center">
                    <span v-if="matchingdata(days, data.day)">{{
                      matchingdata(days, data.day).numberAfternoon
                    }}</span>
                    <span v-else>-</span>
                  </div>
                  <div style="float: left; width: 34%; text-align: center">
                    <span v-if="matchingdata(days, data.day)">{{
                      matchingdata(days, data.day).alreadyNumberA
                    }}</span>
                    <span v-else>-</span>
                  </div>
                  <div style="float: left; width: 33%; text-align: center">
                    <span v-if="matchingdata(days, data.day)">{{
                      matchingdata(days, data.day).stockNumberA
                    }}</span>
                    <span v-else>-</span>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    matchingdata(dayList, dayData) {
      ...
      var start = this.fun_date(
        new Date(this.megliast.startTime.substring(0, 10)),
        this.megliast.leadTime
      );
      ...
      this.$api
        ...
        .then((res) => {
          ...
          that.MSJschedules = that.MSJschedules.reduce(function (item, next) {
            obj[next.scheduleName]
              ? ""
              : (obj[next.scheduleName] = true && item.push(next));
            return item;
          }, []);

          var obj_editableTabs = {};
          that.editableTabs = that.editableTabs.reduce(function (item, next) {
            obj_editableTabs[next.name]
              ? ""
              : (obj_editableTabs[next.name] = true && item.push(next));
            return item;
          }, []);
