                  ...
                  <div style="float: left;margin-left: 150px">
                    <div  v-if="AllHospitalMsg.type==1"v-for="(itemtancan,indexitemtancan) in itemMsg.combos">
                      ...
                      <div v-if="AllHospitalMsg.type==0" style="margin-left:0px;float: left;height: 50px;padding-top: 10px;width: 150px">
                        ...
                        <el-input v-model.trim="itemMsg.quota" maxlength=10000 @change="input_change_leadTime(itemMsg.quota)" placeholder="请输入人数" style="width:130px" clearable></el-input>
                      </div>
...
export default {
  ...
  methods: {
      ...
      if(val.schedules.length>0&&val.schedules){
        ...
        for (var i=0;i<tempSchedules.length;i++){
          if(tempSchedules[i].quota&&tempSchedules[i].quota>0){
            tempt.push(tempSchedules[i].date)
          }
        ...
        tempt=Array.from(new Set(tempt))
        for (var i=0;i<tempt.length;i++){
          if(tempt.length==1){
            time=tempSchedules[0].date
            this.AllHospitalMsg.chooseDAY.push(tempt[i])
          }else if(tempt.length>1){
            time=tempt[0]+"~"+tempt[tempt.length-1]
            this.AllHospitalMsg.chooseDAY.push(tempt[i])
          }
        }
    ...
    submit(){
      ...
      this.$confirm('你确定要生效此条排期吗?', '提示', {
        ...
        this.$network3
          ...
          .then((res)=>{
            if(res.code==200){
              console.log("/ScheduleManagement/ScheduleList")
              that.$router.push('/TuanAdmin/ScheduleAdmin')
    ...
    confirm_kuang() {
      ...
      var PQCCListTemp=[];
      PQCCListTemp=chooseHospitalMsgTempt_mor.concat(chooseHospitalMsgTempt_aft);
      PQCCListTemp.sort((a, b) => a.date.localeCompare(b.date));

      console.log(PQCCListTemp);
      console.log(this.AllHospitalMsg.PQCCList);
        ...
        this.$network3
          ...
          .then(res => {
            ...
            if (res.data) {
              ...
              console.log(this.AllHospitalMsg.days);
              resolve(this.AllHospitalMsg.days)
