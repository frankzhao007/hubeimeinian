  ...
  <div style="margin:20px">
    ...
    <div>
      <el-table :data="tableobj.list" style="width: 100%" v-loading="tableobj.loading">
        ...
        <el-table-column label="奖励详情" min-width="150px" show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-dialog title="新增/编辑活动" :visible.sync="addoreditpop.isshow" width="800px">
          <el-form :model="addoreditpop" label-position='right' label-width="120px" size="medium">
            ...
            <el-form-item label="活动时间：">
              <el-date-picker value-format="yyyy-MM-dd HH:mm:ss" v-model="addoreditpop.timehorizon" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions">
              </el-date-picker>
            </el-form-item>
            ...
            <el-form-item label="福利内容：">
              每个人&nbsp&nbsp<el-input :disabled="addoreditpop.isedit" v-model="addoreditpop.Quota" placeholder="请填写" style="width:80px" @input="Quotainput"></el-input>&nbsp&nbsp次免费体检名额。
            </el-form-item>
          ...
          <!-- <el-dialog width="800px" title="选择套餐" :visible.sync="tableselectenterMeal.isshow" append-to-body>
            ...
            <el-table ref="multipleTable" :row-key="getRowKeys" :data="tableselectenterMeal.tablelist" style="width: 100%" v-loading="tableselectenterMeal.loading" @selection-change="selectActivityEnter">
              ...
              <el-table-column prop="goodprice" label="套餐原价"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    copypagemsg() {
      ...
      this.$store.commit("setFreeMedical", body);
    },
    //赋值列表页面数据
    setpagemsg() {
      var body = this.$store.getters.getFreeMedical;
    ...
    gotoFbselectemp(row) {
      console.log(row);
      this.$router.push("/marketing/Fbselectemp?id=" + row.Id + "&type=2");
    },
    updateActivityEmp() {
      ...
      this.$api.post("/activityConfig/updateFpActivityEmp", body).then(res => {
        console.log(res);
        if (res.code == 200) {
    ...
    findActivityEmp() {
      ...
      this.$api
        .get("/activityConfig/findActivityByEmp", {
    ...
    confirmsetremark() {
      if (!this.remarkpop.remarkmsg || this.remarkpop.remarkmsg.trim() == "") {
      ...
      this.$api
        .post("/activityConfig/updateFpActivity", body)
        .then(res => {
          if (res.code == 200) {
    ...
    delActivity(row) {
      this.$confirm("删除活动后不可恢复，请确认后操作！", "删除确认", {
        ...
        .then(() => {
          this.$api
            .post("/activityConfig/delPyActivity", {
              ids: [row.Id]
            })
            .then(res => {
              console.log(res);
    ...
    addoreditActivity() {
      ...
      if (!this.addoreditpop.Quota || this.addoreditpop.Quota == "") {
        this.$message.error("请完整填写福利内容");
        return;
      }
      if (Number(this.addoreditpop.Quota) < 1) {
        this.$message.error("免费体检名额必须大于1");
        return;
      }
      if (Number(this.addoreditpop.Quota) > 10000) {
        this.$message.error("免费体检名额不可大于10000");
        return;
      }
      var body = {
        ActivityName: this.addoreditpop.ActivityName,
        RewardInfo: {
          Reward: "每人" + this.addoreditpop.Quota + "个名额",
    ...
    findActivitys() {
      ...
      this.$api
        .get("/activityConfig/findEmpFreePhysicals", body)
        .then(res => {
