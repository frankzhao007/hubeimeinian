  ...
  <div class="app-container">
    ...
    <div>
            ...
            <div class="grid_content">
              <el-form :model="confirmseekorder" :rules="confirmseekorderrules" ref="ruleForm" class="demo-ruleForm">
                <el-form-item prop="orNumber">
                  ...
                  <el-input placeholder="请输入订单号" v-model="confirmseekorder.orNumber" clearable size="medium" style="width:100%;"></el-input>
                </el-form-item>
              </el-form>
          ...
          <el-col :span="6">
            ...
            <div class="grid_content">
              <el-select v-model="confirmseekorder.type" placeholder="请选择" size="medium" style="width:100%;" @change="goodstypechange()">
                <el-option v-for="item in seekorder.goodslist" v-if="isshowgoodstype(item.value)" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </div>
          </el-col>
          <el-col :span="6">
            ...
            <div class="grid_content">
              <el-input placeholder="请输入手机号" v-model="confirmseekorder.userPhone" clearable size="medium" style="width:100%;"></el-input>
            </div>
          </el-col>
          <el-col :span="6">
      ...
      <div class="inquire">
        <el-row>
          <el-col :span="6">
            <div class="grid_lable">销售/分销客：</div>
            <div class="grid_content">
              <el-input placeholder="请输入手机号" :disabled="RoleInfo.AccountType=='2'" v-model="confirmseekorder.sellerPhone" clearable size="medium" style="width:100%;"></el-input>
            </div>
          </el-col>
          <el-col :span="6">
            <div class="grid_lable">购买渠道：</div>
            <div class="grid_content">
              <!-- <el-input placeholder="请输入内容" v-model="confirmseekorder.buyChannel" clearable size="medium" style="width:100%;"></el-input> -->
          ...
          <el-col :span="6">
            ...
            <div class="grid_content">
              <el-date-picker v-model="yearSelect" size="medium" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd HH:mm:ss" @change="timechange"></el-date-picker>
            </div>
          </el-col>
        </el-row>
      </div>
      <div class="inquire">
        <el-row>
          <el-col :span="6">
            ...
            <div class="grid_content">
              <el-select v-model="confirmseekorder.joinEnterprise" size="medium" filterable clearable>
                <el-option v-for="item in  joinEnterpriselist" :key="item.id" :label="item.Name" :value="item.Name"></el-option>
              </el-select>
            </div>
          </el-col>
        </el-row>
    ...
    <div>
      <div class="inquire">
        <el-button class="grid_button" type="primary" size="medium" @click="gotoorderexportrecord()" :disabled="!Clickable.export">导出记录</el-button>
        <el-button class="grid_button" type="primary" size="medium" @click="exportordermsg()" :disabled="!Clickable.export">导出</el-button>
        <el-button class="grid_button" type="primary" size="medium" icon="el-icon-search" :disabled="!Clickable.inquire" @click="queryOrderList()">搜索</el-button>
    ...
    <div>
      <el-table :data="orderlist" style="width: 100%">
        ...
        <el-table-column prop="brokerage" label="平台佣金（元）" min-width="120"></el-table-column>
        <el-table-column prop="sellerPhone" label="销售/分销客" min-width="150" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column label="体检状态" min-width="120" show-overflow-tooltip>
          <template slot-scope="scope">{{reStatuschange(scope.row.reStatus)}}</template>
        </el-table-column>
        <el-table-column label="订单状态" min-width="100">
          <template slot-scope="scope">{{goodsStatuschange(scope.row.goodsStatus)}}</template>
        </el-table-column>
              ...
              <el-row>
                <el-col :span="24" v-if="scope.row.goodsStatus==9">
                  <div class="grid-content bg-purple-dark">
                    <el-button :disabled="!Clickable.refund" @click="RefundBtn(scope.$index,scope.row)" type="text" size="small">退款</el-button>
                  </div>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24" v-if="scope.row.goodsStatus==9">
                  <div class="grid-content bg-purple-dark">
                    <el-button :disabled="!Clickable.cancel" @click="CancelrefundBtn(scope.$index,scope.row)" type="text" size="small">取消申请退款</el-button>
                  </div>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24" v-if="scope.row.goodsStatus==2||scope.row.goodsStatus==6">
                  <div class="grid-content bg-purple-dark">
                    <el-button :disabled="!Clickable.delete" @click="ApplyrefundBtn(scope.$index,scope.row)" type="text" size="small">发起退款</el-button>
                  </div>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24" v-if="scope.row.goodsStatus==4">
                  <div class="grid-content bg-purple-dark">
                    <el-button :disabled="!Clickable.inquire" @click="RemitBtn(scope.$index,scope.row)" type="text" size="small">退款打款</el-button>
                  </div>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24" v-if="scope.row.goodsStatus==2&&scope.row.goodsType=='电子体检卡'">
                  <div class="grid-content bg-purple-dark">
                    <el-button :disabled="!Clickable.inquire" @click="changecardmsg('发送卡密',scope.row)" type="text" size="small">发送卡密</el-button>
                  </div>
                </el-col>
              </el-row>
              <el-row>
              ...
              <el-row>
                <el-col :span="24">
                  <div class="grid-content bg-purple-dark">
                    ...
                    <el-button v-if="(scope.row.goodsStatus==2||scope.row.goodsStatus==7)&&scope.row.goodsType=='电子体检卡'" @click="opensendsms(scope.$index,scope.row)" type="text" size="small">发送短信</el-button>
                    <el-button v-if="scope.row.goodsStatus==2&&scope.row.goMsgStatus=='未发送'&&scope.row.goodsType!='电子体检卡'" @click="opensendsms(scope.$index,scope.row)" type="text" size="small">发送短信</el-button>
                    <el-button v-if="scope.row.goodsStatus==2&&scope.row.goMsgStatus!='未发送'&&scope.row.goodsType!='电子体检卡'" @click="opensendsms(scope.$index,scope.row)" type="text" size="small">已发送短信</el-button>
                  </div>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24">
                  <div class="grid-content bg-purple-dark">
                    <el-button :disabled="!Clickable.details" @click="Viewordermsg(scope.$index,scope.row)" type="text" size="small">查看详情</el-button>
                  </div>
                </el-col>
              </el-row>
            </div>
    ...
    <el-dialog title="备注" :visible.sync="SpeakerNote.isshow" width="25%" style="min-width:1200px">
      <el-input type="textarea" :autosize="{ minRows: 3, maxRows: 4}" placeholder="请输入内容" maxlength="40" show-word-limit v-model="SpeakerNote.textarea2"></el-input>
    ...
    <el-dialog title="发起退款" :visible.sync="Applyrefund.isshow" width="30%" style="min-width:1200px">
      <el-row>
        ...
        <el-col :span="18">
          <el-input type="textarea" :autosize="{ minRows: 3, maxRows: 4}" placeholder="请输入内容" maxlength="20" show-word-limit v-model="Applyrefund.refundmsg"></el-input>
        </el-col>
      </el-row>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="退款处理" :visible.sync="Refund.isshow" width="30%" style="min-width:1200px">
      <div>
        ...
        <el-row style="line-height:40px;margin-top:10px;">
          ...
          <el-col :span="16">
            <span>{{selectorder.goodsPay}}</span>
            <span style="margin-left:20px; color:#F63649">{{selectorder.payWelfareName}}</span>
          </el-col>
        </el-row>
        <!-- <el-row style="line-height:40px;margin-top:10px;">
...
<el-col :span="16">未到检</el-col>
        ...
        <el-row style="line-height:40px;margin-top:10px;">
          ...
          <el-col :span="16">
            <el-select size="small" v-model="Refund.RefundResult" placeholder="请选择" clearable style="width:80%">
              <el-option v-for="item in Refund.RefundResultList" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-col>
        </el-row>
    ...
    <el-dialog title="打款处理" :visible.sync="Remit.isshow" width="800px">
      ...
      <div style="padding:30px 20px;color:#2A86FF">
        <div>说明：</div>
        <div>1、确定打款，会将退款金额直接退回到原支付账号，请仔细审核后操作</div>
        <div>2、套餐或者加项退款时，本次购买的加项或者套餐会一起退款</div>
      </div>
      <span slot="footer" class="dialog-footer">
    ...
    <div>
      <el-dialog title="短信发送设置" :visible.sync="SMSobj.isshow" width="700px">
        <el-form :model="SMSobj" label-position="right" label-width="200px" size="medium">
          ...
          <el-form-item label="发送时机：">{{SMSobj.sendOpportunity}}</el-form-item>
          <div style="color:#0466FA;margin:0 30px 10px 200px">说明：每个订单仅可发送一条短信，请注意设置短信内容！</div>
          ...
          <div style="color:#0466FA;margin:0 30px 10px 200px">说明：短信内容为默认模板内容，固定不可编辑内容为“您的购买的{商品名称}商品已经支付成功，”后面可以添加40个字以内！</div>
        </el-form>
    ...
    <div>
      <el-dialog title="短信发送：" :visible.sync="sendSMSobj.isshow" width="600px">
        <el-form :model="sendSMSobj" label-position="right" label-width="180px" size="medium">
          <el-form-item label="购买商品：">{{sendSMSobj.goodsName}}</el-form-item>
          <el-form-item label="发送手机号：">{{sendSMSobj.userPhone}}</el-form-item>
          <el-form-item label="发送状态：">{{sendSMSobj.sendStatus}}</el-form-item>
          <el-form-item label="内容：">
          ...
          <div style="color:#0466FA;margin:0 30px 10px 180px" v-if="sendSMSobj.sendStatus!='未发送'">说明：已发送1条短信不可再次发送！点击确定返回订单详情页短信不会再次发送！</div>
          <div style="color:#0466FA;margin:0 30px 10px 180px" v-if="sendSMSobj.sendStatus=='未发送'">说明：短信文本限制在64个字以内！</div>
        </el-form>
        <div slot="footer" class="dialog-footer">
...
export default {
  ...
  mounted() {
    if (this.$store.getters.getRoleInfo.AccountType == "0") {
      ...
      this.confirmseekorder.filialeValue =
  ...
  methods: {
    ...
    gotoorderexportrecord() {
      this.$router.push("/ordermanagement/orderexportrecord");
    },
    //取消退款申请按钮
    ...
    RefundBtn(index, row) {
      ...
      this.selectorder = row;
      this.Refund.RefundMoney = this.selectorder.goodsPay;
      console.log(row);
