  ...
  <div class="app-container">
    ...
    <div>
      <div class="inquire">
        ...
        <el-button :disabled="!Clickable.add" class="grid_button" type="primary" size="medium" icon="el-icon-plus" @click="AddProjectBtn()">新增体检项目</el-button>
        <el-button :disabled="!Clickable.inquire" class="grid_button" type="primary" size="medium" icon="el-icon-search" @click="seekbtn()">搜索</el-button>
      </div>
    </div>
    <!-- 表格 -->
    <div>
      <el-table :data="projectList" style="width: 100%;" size="medium" v-loading='tableloading'>
        ...
        <el-table-column label="操作" min-width="180" fixed="right">
          <template slot-scope="scope">
            <el-button :disabled="!Clickable.edit" size="small" @click="manageUpdateCheck(scope.row)">编辑</el-button>
            <el-button :disabled="!Clickable.delete" size="small" type="danger" @click="manageDeleteCheck(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.$store.getters.getRoleInfo.AccountType == "0") {
      ...
      this.FilialeList = [];
      this.filialeValue =   this.$store.getters.getRoleInfo.parentName || this.$store.getters.getRoleInfo.AccountName;
  ...
  methods: {
    ...
    //导入记录
    ImportRecord() {
      this.$router.push("/medicalexamination/projectimportrecord");
    },
    exportexecl() {
      var body = {
    ...
    //下载模板
    downloadmoban() {
      // this.$api.get("/manage/downLoadCheckExcel").then(res => {
      //   console.log(res);
      ...
      xhr.open(
        "GET",
        this.$parent.$parent.$parent.weburl+"/manage/DownLoadFile?id=1",
        true
      ); //也可以使用POST方式，根据接口
        ...
        if (this.status === 200) {
          ...
          if (blob.size > 0) {
            ...
            reader.onload = function(e) {
              ...
              a.download =
                "项目模板" + newDate.Format("yyyyMMddhhmmss") + ".xlsx";
        ...
        reader.onload = function(e) {
          ...
          outdata.map(res => {
            let obj = {
              xuhao: res.序号,
              areaname: res.区域,
              areanameid: res.区域项目代码,
              instcity: res.城市,
              instcityid: res.城市项目代码,
              checktypefirst: res.一级类目,
              checktypesec: res.二级类目,
              checkname: res.单项名称,
              checkdescribe: res.详细描述,
              checkgender: res.性别,
              checkmarstatus: res.婚否,
              checkprice: res.门市价,
              checklowprice: res.核算价,
              checksignificance: res.临床意义,
              checkspecial: res.特殊说明
            };
            body.checks.push(obj);
          });
          _this.$api
            .post("/manage/manageImportChecks", body)
            .then(res => {
              console.log(res);
              // _this.innerVisible = true;
              loading.close();
              _this.$message.success("导入体检项目成功！");
              _this.toleadisshow = false;
              _this.ProjectListQuery();
            })
            .catch(res => {
              console.log(res);
              _this.innerVisible = true;
    ...
    //新增体检项目按钮
    AddProjectBtn() {
      this.$router.push("/medicalexamination/addproject?ischange=false");
    },
    //区域转换
    changearea(str) {
      switch (str) {
    ...
    ProjectListQuery() {
      ...
      var body = {
        ...
        areanameid: this.projectseek.ProjectAreaID,
        instcity: this.projectseek.ProjectCity,
        instcityid: this.projectseek.ProjectCityID.trim(),
        page: this.projectcurrentPage,
        pagesize: this.projectpageSize,
        instname: this.filialeValue
      };
      this.$api.post("/manage/managequerychecks", body).then(res => {
        if (res.code == "200") {
    ...
    manageUpdateCheck(row) {
      console.log(row);
      this.$router.push(
        "/medicalexamination/addproject?ischange=true&checkid=" + row.checkid
      );
    },
